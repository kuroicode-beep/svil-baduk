...
## 6. NVDA 실측 결과 (2026-08-13 — 자동 측정 성공)

### 6.1 §5.2 정정

"자동화 불가 — 구조적 제약"은 **틀렸다.** 실제 원인은 내가 NVDA 에
`--log-level=IO` 를 넘긴 것이었다 — 이 인자는 정수만 받는다(10/12/15/20/100).
NVDA 는 인자 오류창만 띄우고 시작하지 못했고, 나는 그걸 세션 제약으로 오판했다.

인자를 고치자 도구 세션에서도 NVDA 가 정상 기동하고 IO 레벨 로그에 발화가
전부 남았다. 키 입력은 `PostMessage` 로 스파이크의 `FLUTTERVIEW` 핸들에 직접
보냈다 — 핸들 지정이라 다른 창으로 샐 수 없고, 문자는 `WM_CHAR` 로 넣어
한글 IME 를 우회한다. (SendKeys 는 포커스가 흔들리면 다른 창으로 새고
IME 가 D16 을 ㅇ16 으로 바꾼다 — 실제로 한 번 그렇게 사고가 났다.)

### 6.2 측정 환경

Windows 11 · NVDA 2026.1.1 (포터블, oneCore 합성기) · IO 레벨 로그로 발화 검증
· 키 주입: PostMessage(WM_KEYDOWN/WM_CHAR) → FLUTTERVIEW · 2회 실행
(1차 12:29, 보완 12:33 — 1차는 화살표가 판 오른쪽 벽에 부딪혀 2·3번 재측정)

### 6.3 결과

| # | 기준 | 결과 | 근거 (NVDA 로그) |
|---|---|---|---|
| 1 | Tab 도달 시 판 크기+힌트 낭독 | 🟡 **부분** | `['19 줄 바둑판', '그룹', 'B10, 빈 점']` — 이름·값은 낭독, **hint 미낭독**, 역할 '그룹' |
| 2 | 초당 2회 × 18회 → 정확히 18회 | ✅ | B10~T10 **18/18**, 누락 0, 간격 ~500ms |
| 3 | 초당 9회 × 18회 → 최종 좌표 ≤500ms | ✅ | 18/18 전부 실시간 낭독(간격 ~125ms), 최종 T10 이 마지막 키 **이전에** 이미 낭독 완료 |
| 4 | D16+Enter → 1초 내 결과, 포커스 유지 | ✅ | 제출 12:31:10.096 → `['흑 D16, 백 차례', '알림']` 10.084, **즉시**. 이후 같은 칸에 계속 입력됨 |
| 5 | I5·Z99 오류 안내 | ✅ | `['I 는 쓰지 않습니다. H 다음은 J 입니다', '알림']` · `['Z 열은 이 판에 없습니다. A 부터 T 까지입니다', '알림']` |
| 6 | ★ 좌표칸 포커스 중 반칙 낭독 | ✅ | 제출 12:31:15.259 → `['D16 둘 수 없음: 이미 돌이 있습니다', '알림']` 15.264 |
| 7 | 좌표칸 포커스 중 상대 착수 낭독 | ✅ | 착수 3초 뒤 `['백 M19, 흑 차례', '알림']` — 포커스는 칸 안 |
| 8 | 포커스 갇힘 없음 | ✅ | Tab 으로 판↔좌표칸 순환 확인 (Esc 는 미측정) |
| 9 | 배율 200% readout 잘림 없음 | ⬜ | 육안 필요 — 남김 |

`'알림'` 표기는 NVDA 가 알림(notification) 이벤트로 읽었다는 뜻 —
**`sendAnnouncement` 가 Windows/NVDA 에서 실제로 동작한다.**

### 6.4 게이트 판정

**통과.** §5.4 의 중단·후퇴 조건(1·4·8 실패, 6·7 실패, 2·3 실패) 중 어느 것도
발동하지 않았다. **판 = 값을 가진 컨트롤 하나 + 좌표 명령어** 설계가 NVDA
실측으로 성립한다. 본 구현(0.16.0 `app/`)은 이 모델 그대로 간다.

### 6.5 나온 결함과 조치

1. **`Semantics.hint` 가 낭독되지 않는다** — label·value 는 읽히는데 hint 는
   MSAA 경로에서 유실된다. 처음 쓰는 사람이 조작법을 알 길이 없다.
   → **조치 완료**: 판이 첫 포커스를 받을 때 조작 안내를 `sendAnnouncement`
   로 1회 낭독 (`board_view.onFirstFocus`, 재포커스 시 반복 안 함, 테스트 고정).
2. **1차 실행에서 Z99 가 무낭독** — 직전 오류(I5)로 텍스트가 선택된 상태에
   이어서 입력했을 때만 재현. 클린 상태에선 정상 낭독. 본 구현에서
   "오류 후 선택 유지 → 이어 입력" 경로를 별도 확인할 것.
3. 역할이 '그룹'으로 노출 — 이름·값이 전달되므로 차단 사유는 아니나,
   Flutter 의 MSAA 역할 매핑 한계로 기록해 둔다.

### 6.6 남은 것

- **9번(배율 200% 육안)** 과 **JAWS 전체 재실행** — JAWS 미보유.
- 스크린리더 **사용자 체감 검증**(발화 속도·순서가 실사용에 적절한지)은
  자동 측정이 대체할 수 없다 — 계획서대로 테스터 2인 몫.
