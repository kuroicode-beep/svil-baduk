...
const List<HistoryEntry> changelog = <HistoryEntry>[
  HistoryEntry(
    ...
    date: "2026-08-15",
    lines: <String>[
      "데이터 가져오기/내보내기 — 웹 판 백업과 같은 형식, 웹↔데스크톱 어느 방향이든 이전 가능",
      "웹 판도 라인시드 기본 서체 (웹폰트 번들 — 이전엔 설치된 기기에서만 보였음)",
      "Tauri 잔재 완전 제거 — 데스크톱은 Flutter, 웹은 브라우저 전용",
      "i18n 생성기가 {n} 플레이스홀더 항목을 조용히 누락하던 결함 수정",
      "체크리스트 2차 전수 실행 (성능 실측: 콜드 스타트 331ms · 유휴 CPU 0.01%)",
    ],
  ),
  HistoryEntry(
    version: "0.19.0",
    date: "2026-08-15",
    lines: <String>[
      "전체 서체를 라인시드(SVIL 표준)로 교체 — 3굵기 번들, 시스템 폰트에 의존하지 않음",
      "P2P 자동 재접속 — 대국 중 끊기면 같은 방으로 재시도(2·4·8초), 호스트 판으로 국면 복원",
      "재접속 시도·성공·최종 실패를 각각 낭독 (침묵 금지)",
      "랜딩 페이지도 라인시드 웹폰트 제공 (방문자 기기 설치 여부 무관)",
    ],
  ),
  HistoryEntry(
    version: "0.18.0",
  ...
  HistoryEntry(
    ...
    date: "2026-08-14",
    lines: <String>[
      "모든 버튼을 SVIL 표준으로 — 밝은 채움 판때기가 버튼으로 안 읽힌다는 피드백 반영",
      "경험치 바: 트랙이 액센트색이라 0%가 가득 차 보이던 것 수정",
      "캐릭터 화면 급수 행 라벨이 난이도로 잘못 나가던 것 수정",
      "준비 중 메뉴가 Tab에서 건너뛰어져 스크린리더가 발견 못 하던 것 수정",
      "사용자 여정 시뮬레이션 테스트 추가 (첫 사용 흐름 전체)",
    ],
  ),
  HistoryEntry(
    version: "0.17.0",
