...
Write-Host 'Downloading KataGo v1.16.5 OpenCL (Windows x64)...' -ForegroundColor Cyan
gh release download v1.16.5 --repo lightvector/KataGo `
  --pattern 'katago-v1.16.5-opencl-windows-x64.zip' `
  --dir $Tmp --clobber

$zip = Get-ChildItem (Join-Path $Tmp 'katago-v1.16.5-opencl-windows-x64.zip') | Select-Object -First 1
...
$cfg = $cfg.Replace('maxVisits = 500', 'maxVisits = 64').Replace('numSearchThreads = 6', 'numSearchThreads = 4')
...
Write-Host '  (or) npm run dev:all'
Write-Host 'App auto-connects when katagoAutoConnect is on (default).'
