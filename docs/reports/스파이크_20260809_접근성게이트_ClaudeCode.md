...
## 5. 실측 준비 완료 (2026-08-13 추가)

실측을 자동화하려 시도했고, **자동화가 불가능하다는 것을 확인**했다. 대신 사람이 할 일을
한 번의 더블클릭으로 줄여 놓았다.
...
| 항목 | 위치 |
|---|---|
| 스파이크 빌드 | `spikes\board_a11y\build\windows\x64\runner\Release\board_a11y.exe` |
| NVDA 2026.1.1 (포터블) | `D:\C_Backup\nvda-portable\app\nvda.exe` |
| NVDA 정식 설치본 | `D:\C_Backup\nvda-portable\nvda_2026.1.1.exe` |
| 실행 스크립트 | `spikes\board_a11y\run_measurement.ps1` |
| 바탕화면 바로가기 | `SVIL Baduk 접근성 실측.lnk` |

NVDA 는 시스템에 설치하지 않고 **포터블 복사본**으로 두었다. 폴더를 지우면 흔적이 남지 않는다.
받은 파일의 서명은 `CN=NV Access Limited` 로 **유효**했다(공식 배포본 확인).

### 5.2 자동화가 안 되는 이유 (실측으로 확인)

NVDA 를 무음 합성기 + IO 로그 레벨로 띄워 발화 내용을 파일로 받으려 했다.
프로세스는 뜨지만 **로그를 한 줄도 쓰지 않는다.** 두 번 시도해 같은 결과였다.

원인은 NVDA 가 **대화형 데스크톱 세션에 붙어야** 동작한다는 데 있다. 도구가 띄우는
분리된 프로세스에서는 화면을 후킹하지도, 세션 로그를 만들지도 못한다. 이 앱의
스크린샷을 찍을 수 없는 것과 같은 종류의 제약이다.

즉 **이 항목이 "사용자 확인 필요" 목록에 있는 것은 게으름이 아니라 구조적 제약**이다.
체크리스트에 넣지 않고 목록 밖에서 관리하는 이유이기도 하다.

### 5.3 실측 절차

1. 바탕화면 **`SVIL Baduk 접근성 실측`** 을 더블클릭한다 (NVDA + 스파이크가 함께 뜬다).
2. NVDA 메뉴(`Insert+N`) → **도구 → 음성 뷰어**를 켠다. 낭독된 문장이 텍스트로 쌓인다.
3. 아래 §3 표의 항목을 순서대로 확인하고 결과를 적는다.
4. **6번이 가장 중요하다** — 이 하나가 바둑판 설계의 분기점이다.

### 5.4 결과에 따른 분기 (미리 정해 둔다)

| 실패 항목 | 해야 할 일 |
|---|---|
| **6 또는 7** | 오류도 `value` 채널로 미러링 + 오류 시 판으로 포커스 왕복. 그래도 안 되면 좌표 입력칸이 유일한 포커스 소유자가 되고 모든 상태를 그 `value` 에 싣는다 |
| **2 또는 3** | 화살표 커서를 **저시력 시각 보조 전용**으로 강등하고, 좌표 명령어를 유일한 시각장애 조작 모델로 삼는다. 여전히 좋은 제품이지만 **날짜가 적힌 문서화된 결정**이어야 한다 |
| **1 · 4 · 8** | **중단.** Flutter 는 이 제품의 스크린리더 경로를 Windows 에서 담지 못한다는 뜻이다. Flutter 는 저시력·확대경 전용 데스크톱 클라이언트로 축소하고, 스크린리더 사용자는 React 웹을 쓴다 |
