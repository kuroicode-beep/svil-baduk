...
void main() {
  group('지점 낭독', () {
    test('좌표가 항상 문장 맨 앞에 온다', () {
      ...
      expect(testSpeech.point(g, 3, 3).startsWith('D16'), isTrue);
      expect(testSpeech.point(g, 0, 18).startsWith('A1'), isTrue);
      expect(testSpeech.point(g, 15, 15, detail: SpeechDetail.full).startsWith('Q4'),
          isTrue);
    });
    ...
    test('빈 점·돌·화점을 구분한다', () {
      ...
      expect(testSpeech.point(g, 3, 3), 'D16, 흑, 화점');
      expect(testSpeech.point(g, 15, 15), 'Q4, 백, 화점');
      expect(testSpeech.point(g, 5, 5), 'F14, 빈 점');
    });

    test('직전 수를 표시한다', () {
      final GameState g = createGame(BoardSize.s19);
      final String s = testSpeech.point(g, 4, 4, lastMove: const Point(4, 4));
    ...
    test('자세히 모드에서 돌의 활로를 센다', () {
      ...
      expect(testSpeech.point(g, 0, 0, detail: SpeechDetail.full), contains('활로 2'));
    });
    ...
    test('자세히 모드에서 빈 점의 이웃을 읽는다', () {
      ...
      final String s = testSpeech.point(g, 4, 4, detail: SpeechDetail.full);
    ...
    test('간단 모드는 활로·이웃을 말하지 않는다', () {
      ...
      expect(testSpeech.point(g, 0, 0), isNot(contains('활로')));
    });
  });
  ...
  group('A6 · 착수 결과', () {
    test('색·좌표·다음 차례를 말한다', () {
      ...
      final String s = testSpeech.moveResult(r.state, r.move);
      expect(s, '흑 D16, 백 차례');
    });
  ...
  group('A13 · 줄·요약 질의', () {
    test('한 줄의 돌을 읽는다', () {
      ...
      // 9줄 판의 y=0 은 9줄
      final String s = testSpeech.row(g, 9);
    ...
    test('빈 줄을 알린다', () {
      ...
      expect(testSpeech.row(g, 5), contains('돌 없음'));
    });
    ...
    test('판 요약에 돌 수와 차례가 들어간다', () {
      ...
      final String s = testSpeech.summary(g, lastMove: const Point(4, 4));
    ...
    test('직전 수가 없으면 그렇게 말한다', () {
      expect(testSpeech.summary(createGame(BoardSize.s9)), contains('직전 수 없음'));
    });
  });
}
